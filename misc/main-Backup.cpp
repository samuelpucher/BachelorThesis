...
int main(){
    ...
    */
    ...
    ofstream file_f;
    file_ft.open ("./");
    file_g.open ("./16.12/function_g_phi_pi.txt");
    file_f.open ("./16.12/function_f_phi_pi.txt");
            ...
            for (int i = 0; i < GRID_R; i++)
            {
                ...
                if (t==5000)
                {
                    for (int j = 0; j < GRID_R; j++)
                    {
                        ...
                        file_f << get_r(i) << " " << get_r(j) << " " << arr_function_f[i][j][phi_half] << " " << arr_Vf[i][j][phi_half]<< " "<< arr_potential_U[i][j][phi_half] << "\n";      //Sollte 1 für große i und klein für kleine i               
                    }
                }           
            }            
            file_g << "\n"  << std::flush;
